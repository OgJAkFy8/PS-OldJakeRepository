...
<#Function Old-Info-COOPs-Snaps {
    ...
    Write-Host `n "Snapshot information of all VM's in our vsphere." -foreground Yellow -BackgroundColor Black
    Write-Host '=============================' -foregroundcolor Yellow
    $Snapshotinfo | Sort-Object Created,SizeGB -Desc | Format-Table -Property VM,Name,Created,@{n='SizeGb';e={'{0:N2}' -f $_.SizeGb}}#, id -AutoSize
    Write-Host '=============================' -foregroundcolor Yellow
    }
    }
#>
...
Function Create-COOPs {
	...
	function Get-VirtualMachines
 {
		...
		process
		{
			...
			if ($InputObject.Name -like $TDStamp + '-COOP.*')
			{
				$InputObject
			}
		}
	...
	Write-Host -Separator `n $VMServer | Format-Table Name,ResourcePool -AutoSize 
	...
	switch ($OkAdd){
		Y {
			foreach ($server in $VMserver) {
				...
				Write-Host '=============================' -foregroundcolor Yellow
				### Get-Virtualmachines function 
				Get-VirtualMachines | Format-Table Name
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU4zxKThPsRCMp+jwVOhsukJlV
# ysCgggINMIICCTCCAXagAwIBAgIQyWSKL3Rtw7JMh5kRI2JlijAJBgUrDgMCHQUA
...
# KwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQURBsNhjjquwxeCexx9evCRmeoPsEw
# DQYJKoZIhvcNAQEBBQAEgYCMqzDY5xi+redMQCH2EiXx2AWZltTUl6ew5eykEtY1
# mlyIp4nEgGsJmXoO0+3jR4I2q3ZLnXVgeUCJgFuW5IhaLvpQfOfFRw3/R5QoNKkU
# k1BKda4oec14NJq8Ob/dVWvG1+y6XXWgDsgVrk4yKz/JBYLMoZxeE7v7+VbIX80L
# 4A==
