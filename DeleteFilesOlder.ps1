...
# Service Name
$Script:Service = Get-Service Eventloganalyzer  #Future option make this a null option in cases that it does not need to be stopped.
...
function f_fileTest($Outputfile){
if(!(test-path $Outputfile)){New-Item $Outputfile -type file -Force}
}
function f_foldTest($Outputfold){
...
#Write-EventLog –LogName "PS Test Log" –Source “My Script” –EntryType Information –EventID 100 –Message “This is a test message.”
    ...
    "Start Time: $strtTime" | Out-File $Outputfile -Append
    #"Amount of days to save: $dayLimit" | Out-File $Outputfile -Append
    "Deleted files created before: $limit" | Out-File $Outputfile -Append

    if($fileCount -gt 4){
        "Folders deleted: $fileCount" | Out-File $Outputfile -Append
        "Space recovered: $spaceRecovered GB" | Out-File $Outputfile -Append
    }
    # Elapsed Time
    "Elapsed Time: $(($stopTime-$strtTime).totalseconds) seconds" | Out-File $Outputfile -Append
    #"Working Stop Time - $stopTime" | Out-File $Outputfile -Append
    "Run by: $env:USERNAME" | Out-File $Outputfile -Append
