...
<#
    .SYNOPSIS
    Removes and/or Creates copies (COOPs) of our servers
...
Function Get-VMinformation {
  ...
  function Where-LocalDataStore
  {
    ...
    process
    {
      if ($InputObject.name -like 'LOCALdatastore18')
      {
        $InputObject
      }
    }
  ...
  $Script:VMDataStoreStore = Get-Datastore | Where-LocalDataStore


  # Script Variables
  $Script:HostsAll = get-vmhost # All hosts controlled by vcenter
  $Script:HostsLocal = get-vmhost | Where-Object {$_. } # All hosts in current site
...
Function Info-COOPs-Snaps {
  ...
  If ($Snapshotinfo.count -ne 0){
    Write-Host -sep `n $Snapshotinfo | Select-Object -Property Yellow
  }
  else{
...
Function Old-Info-COOPs-Snaps {
  ...
  $Snapshotinfo = get-vm | get-snapshot | Sort-Object Created,SizeGB -Desc | Select-Object -Property VM,Name,Created,@{n='SizeGb'
...
Function Remove-COOPs {
  ...
  $VMServers | Select-Object -Property Name
  ...
  $PoweredOffClones = $VMPoweredOff | Where-Object {$_.Name -like ('{0}*' -f $COOPdate)} #| ft Name, ResourcePool  -AutoSize
...
Function Create-COOPs {
  ...
  $VMServer = VMpoweredON | Where-Object{($_.name -match 'gm') -and ($_ -tag 'COOPdr')}
